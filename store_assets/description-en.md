# Short Description (80 chars max)
Swipe to clean your gallery — delete, keep, queue. Fully local & privacy-first.

# Long Description
Picme is the fastest way to clear out photos and videos cluttering your phone.

**How it works:**
- Swipe left → add to delete queue
- Swipe right → keep
- When ready, bulk-delete the queue in one tap

**Key features:**
✓ All Media, Photos, Videos, Screenshots, Downloads categories
✓ Newest / Oldest / Largest sort options
✓ Delete queue — review everything before committing
✓ Undo — reverse your last swipe any time
✓ Queue and kept items persist across app restarts
✓ Delete history log

**Privacy first:**
Picme never uploads any photo or video to a server. Gallery scanning is entirely on-device. Deletion only happens with your explicit confirmation.

Make gallery cleanup feel like a game!
